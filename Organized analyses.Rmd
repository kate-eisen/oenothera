...
```{r}
Arizona <- ER_f_mass %>% filter(Population=="Arizona") %>%  select(starts_with("Comp."))
Zion <- ER_f_mass %>% filter(Population=="Zion") %>%  select(starts_with("Comp."))
Inyo <- ER_f_mass %>% filter(Population=="Inyo") %>%  select(starts_with("Comp."))
Logan <- ER_f_mass %>% filter(Population=="Logan") %>%  select(starts_with("Comp."))
Idaho <- ER_f_mass %>% filter(Population=="Idaho") %>%  select(starts_with("Comp."))
...
legend(x=-1.5, y=-0.2, legend=levels(as.factor(ER_f_mass_groups$Population)),  col=colors, pch=c(15,15,15,15,15), cex=1)
...
#text(pl, "sp", scaling=1, arrow=TRUE, length=0.5, col=4, cex=0.6, xpd=TRUE)
legend(x=-1.4, y=-0.4, legend=levels(as.factor(ER_f_mass_groups$Population)),  col=colors, pch=c(15,15,15,15,15), cex=1)
...
c.table<-as.data.frame(cbind(compounds,cor1=co1,p1=pvals.a,cor2=co2, p2=pvals.a2))
c.table$cap1<-as.numeric(as.character(c.table$p1))
c.table$cap2<-as.numeric(as.character(c.table$p2))
c.table$sig1<-rep("No", times=dim(c.table)[1])
c.table$sig2<-rep("No", times=dim(c.table)[1])
c.table$sig1<-ifelse(c.table$p1 < 0.01, "Yes", c.table$sig1)
c.table$sig2<-ifelse(c.table$p2 < 0.01, "Yes", c.table$sig2)
...
#text(pl, "sp", scaling=1, arrow=TRUE, length=0.5, col=4, cex=0.6, xpd=TRUE)
legend(x=-0.7, y=-0.1, legend=levels(as.factor(All_data_names$Population)),  col=colors, pch=c(15,15,15,15,15), cex=1)
...
co1<-sapply(1:49,function(x) cors[,x]$estimate)
co2<-sapply(1:49,function(x) cors2[,x]$estimate)
...
## For repeatability
`P_A_totals` has the data for compound diversity
`ER_f_mass` has the total emission rate
...
```{r}
rpt_est <- rptGaussian(Comp.Total~(1|Population)+(1|Pop_Plant), grname=c("Pop_Plant","Population"), data=ER_f_mass)
```
